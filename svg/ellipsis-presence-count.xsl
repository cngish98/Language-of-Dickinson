...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg"
    ...
    <xsl:template name="xsl:initial-template">
        <svg viewBox="-150 100 900 600" width="100%" class="ellipsis-graph">
            <g> <!--transform="translate(80,600)"-->
                ...
                <line x1="0" y1="-{$max-height * $y-scale div 4}" x2="{$max-width}"
                    y2="-{$max-height * $y-scale div 4}" stroke="grey" stroke-dasharray="4 4 4"
                    stroke-width="2"/>
                <line x1="0" y1="-{$max-height * $y-scale div 2}" x2="{$max-width}"
                    y2="-{$max-height * $y-scale div 2}" stroke="grey" stroke-dasharray="4 4 4"
                    stroke-width="2"/>
                <line x1="0" y1="-{$max-height * $y-scale * .75}" x2="{$max-width}"
                    y2="-{$max-height * $y-scale * .75}" stroke="grey" stroke-dasharray="4 4 4"
                    stroke-width="2"/>
                <line x1="0" y1="-{$max-height * $y-scale}" x2="{$max-width}"
                    y2="-{$max-height * $y-scale}" stroke="grey" stroke-dasharray="4 4 4"
                    stroke-width="2"/>

                <!-- axes (x, then y) -->
                <line x1="0" x2="{$max-width}" y1="0" y2="0" stroke="black" stroke-width="2"
                    stroke-linecap="square"/>
                <line x1="0" x2="0" y1="0" y2="{-$max-height * $y-scale}" stroke="black"
                ...
                <text x="-50" y="-{$max-height * $y-scale div 2}" text-anchor="middle"
                    writing-mode="tb" font-size="x-large">Total number of poems in our corpus</text>

                <!-- title (x-axis) -->
                <text x="{$max-width div 2}" y="60" text-anchor="middle" font-size="x-large"
                    >Ellipsis presence?</text>

                <xsl:for-each-group select="$poems-corpus//poem/metadata/ellipsis_present"
                    group-by=".">
                    ...
                    <xsl:variable name="midbar-pos" as="xs:double"
                        select="$x-pos + $bar-width div 2"/>
                    ...
                    <rect x="{$x-pos}" y="-{$ellipsis-count * $y-scale}"
                        height="{$ellipsis-count * $y-scale}" width="{$bar-width}" stroke="black"
                        stroke-width=".5" fill="{$bar_color}"/>
                    ...
                    <text x="{$midbar-pos}" y="-{$bar_height + 10}" text-anchor="middle"
                        fill="black" font-size="medium">
